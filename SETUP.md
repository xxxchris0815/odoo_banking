...
| Online Bank Statements: ZEN.COM | `account_statement_import_online_zen` | **19.0.1.8.0** |
...
| ZEN 500 INTERNAL_SERVER_ERROR | Steht so in der History-Doku. History ist `GET /payments/v1.0/history` mit `accountId` + `createdAt*` + `bookedAt*` (`yyyy-MM-dd`, To inklusiv, nicht in der Zukunft). Details bleiben `GET /payments/v1.0/{uuid}`. |
